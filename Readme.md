...
(async () => {
        ...
        try {
            await client.invoke(
                new Api.messages.ForwardMessages({
                    fromPeer: new Api.InputPeerChannel({ channelId: 1866908990, accessHash: 6722354282051457751n }), // pass an InputPeerChannel object with the channel ID and access hash 
                    id: [element], // pass an array with the ID of the message that you want to forward 
                    randomId: [BigInt(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER))], // generate a new random ID for each message 
                    toPeer: new Api.InputPeerChannel({ channelId: 2136201334, accessHash: 2746950463605601577n }), // pass an InputPeerChannel object with the destination channel ID and access hash 
                })
            );
    ...
    // const channel = await client.getEntity(new Api.PeerChannel({ channelId: 2136201334 }));
    // console.log(channel.id); // this will print the channel ID
    // console.log(channel.accessHash); // this will print   
